...
One provider per secret, item path buried in args. With `jsonOnly: false`, each provider outputs a raw value: one provider can only serve one secret. Add a third API key and you're copy-pasting yet another block.
...
OpenClaw has a "skills" system: SKILL.md files that tell the agent how to perform specific tasks. I packaged the shim and setup instructions as a skill so the agent knows where the script lives, understands the protocol, and can wire new secrets into the config without me explaining it every time. Want to add a new API key? The agent reads the skill, creates the 1Password item, and updates `openclaw.json` with no manual steps.
